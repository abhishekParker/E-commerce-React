...
import { addCartItem, fetchDailyData } from "../store/cartSlice";
...
const HomeDetail = () => {
  ...
  const prod = homeData.filter(({ id }) => id === parseInt(params.id));

  return (
    <>
      {prod.length > 0 && (
        <div className="w-full h-screen flex px-36 py-10 space-x-8 bg-gray-200 justify-center">
          <div className="px-20 py-10 border-[1px] border-gray-400 bg-white w-[30%] h-[70%] flex justify-center items-center shadow-xl bg-cover">
            <img
              src={prod[0].images[0]}
              alt={prod[0].title}
              className="object-cover hover:scale-150 duration-300 bg-cover"
            />
          </div>
